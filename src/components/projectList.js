...
class ProjectList extends Component {
    ...
    componentWillMount(){
        ...
        $.ajax({
            type: 'get',
            url: 'http://localhost:8080/getUrlList' ,
            data: {},
            success: (data) => {
                _this.setState({data: data, totalPage:Math.ceil( data.length/_this.state.pageSize), indexList: data.slice(_this.state.num,_this.state.num+_this.state.pageSize)});
            } ,
            dataType: 'jsonp',
            ...
            <div>
                ...
                <div className="s-body">
                    {this.state.indexList.map((d, index) => {
                        return <div key={index} className={`pl-img-l ${this.state.i === index ? 'active' : ''}`}>
